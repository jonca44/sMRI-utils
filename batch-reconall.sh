...
function reconall_position_select() {
	...
	if [ $choice -eq 1 ]; then 
		echo "Enter subject ID.  " ; read subjid ; echo "Enter path to the subject's first .dcm or .nii file for his/her T1 MPRAGE RMS scan" ; read dcmpath ; echo "Starting a full recon-all for subject number $subjid" ; gnome-terminal --tab --title="recon-all; subjid: $subjid" -e "/bin/bash -c '. chb-fs stable-5.0 ; mosbatch recon-all -i $dcmpath -subjid $subjid ; cp xopts.txt ${subjid}/scripts/expert.opts ; mosbatch recon-all -expert ${subjid}/scripts/expert.opts -xopts-overwrite -s $subjid -mprage -all -gca RB_all_2011-10-25.gca -gca-skull RB_all_withskull_2011-10-25.gca -gca-dir /chb/sheridanlab/software/etc -nowmsa'"
	elif [ $choice -eq 2 ]; then
		echo "Enter subject ID.  " ; read subjid ; echo "Starting recon-all from -autorecon2-cp to the end for subject number $subjid" ; gnome-terminal --tab --title="-autorecon2-cp -autorecon3; subjid: $subjid" -e "/bin/bash -c '. chb-fs stable-5.0 ; cp xopts.txt ${subjid}/scripts/expert.opts ; mosbatch recon-all -expert ${subjid}/scripts/expert.opts -xopts-overwrite -s $subjid -autorecon2-cp -autorecon3 -gca RB_all_2011-10-25.gca -gca-skull RB_all_withskull_2011-10-25.gca -gca-dir /chb/sheridanlab/software/etc -nowmsa'"
	elif [ $choice -eq 3 ]; then 
		echo "Enter subject ID.  " ; read subjid ; echo "Starting recon-all from -autorecon2 to the end for subject number $subjid" ; gnome-terminal --tab --title="-autorecon2 -autorecon3; subjid: $subjid" -e "/bin/bash -c '. chb-fs stable-5.0 ; cp xopts.txt ${subjid}/scripts/expert.opts ; mosbatch recon-all -expert ${subjid}/scripts/expert.opts -xopts-overwrite -s $subjid -autorecon2 -autorecon3 -gca RB_all_2011-10-25.gca -gca-skull RB_all_withskull_2011-10-25.gca -gca-dir /chb/sheridanlab/software/etc -nowmsa'"
	elif [ $choice -eq 4 ]; then 
		echo "Enter subject ID.  " ; read subjid ; echo "Starting recon-all from -autorecon2-wm to the end for subject number $subjid" ; gnome-terminal --tab --title="-autorecon2-wm -autorecon3; subjid: $subjid" -e "/bin/bash -c '. chb-fs stable-5.0 ; cp xopts.txt ${subjid}/scripts/expert.opts ; mosbatch recon-all -expert ${subjid}/scripts/expert.opts -xopts-overwrite -s $subjid -autorecon2-wm -autorecon3 -gca RB_all_2011-10-25.gca -gca-skull RB_all_withskull_2011-10-25.gca -gca-dir /chb/sheridanlab/software/etc -nowmsa'"
